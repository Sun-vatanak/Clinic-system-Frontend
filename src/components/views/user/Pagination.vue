<template>
    <div class="row gy-3">
        <div v-if="!userStore.total_records == 0" class="col-12 ps-3 col-md-6 d-flex justify-content-center justify-content-md-start mb-3">
            <span>បង្ហាញ {{ (userStore.currentPage - 1) * userStore.perPage + 1 }} ដល់
                {{ Math.min(userStore.currentPage * userStore.perPage, userStore.total_records) }}
                នៃ
            ...
            <nav v-if="totalPages > 1" aria-label="Page navigation" class="mt-3">
                <ul class="pagination">
                    <li class="page-item" :class="{ disabled: currentPage === 1 }">
                        <a class="page-link" href="#" @click.prevent="userStore.changePage(currentPage - 1)">
                            &laquo;
                        </a>
                    </li>
                    ...
                    <li v-if="visiblePages[visiblePages.length - 1] < totalPages" class="page-item">
                        <a class="page-link" href="#" @click.prevent="userStore.changePage(totalPages)">{{
                            totalPages }}</a>
                    </li>
...
const { currentPage, totalPages } = storeToRefs(userStore);
