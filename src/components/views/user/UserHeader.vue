<template>
  <div
    class="row rounded-14 d-flex align-items-center justify-content-between section-spacing"
  >
    ...
    <div class="col-12 col-md-8 p-0">
      ...
      >
        <div
          class="bg-white search-width mb-3 mb-md-0 search-bar d-flex align-items-center ps-2 rounded-5"
        >
          <input
            type="search"
            @input="userStore.onloadUser()"
        ...
        <div>
          <select
            class="form-select select-width sort-user px-3 fs-16 text-dark-emphasis search-bar border-0 rounded-5"
            v-model="userStore.selectedRole"
            @change="userStore.onloadUser()"
          >
            <option value="" disabled selected>ជ្រើសរើស</option>
            <option value="">ទាំងអស់</option>

            <option value="1">អ្នកគ្រប់គ្រង​ជាន់ខ្ពស់</option>
            <option value="2">វេជ្ជបណ្ឌិត</option>
            <option value="3">បុគ្គលិក</option>
        ...
        <div class="d-flex align-items-center gap-3">
          <button
            ...
            class="btn btn-primary p-0 btn-add"
          >
            <i class="bi bi-plus-lg"></i>
          </button>
          <button
            class="btn btn-primary rounded-5 px-3 d-flex align-items-center"
            @click="exportToCSV"
          >
...
const exportToCSV = async () => {
  try {
    const response = await axios.get(
      `/api/users?all=true&role_id=${userStore.selectedRole}`
    );
    if (response.data.result) {
      ...
      const data = allData.map((item) => [
        `"${item.profile?.first_name || ""} ${item.profile?.last_name || ""}"`.trim() || "N/A",
        `"${item.email || "N/A"}"`,
        `"${item.profile?.phone || "N/A"}"`,
          item.profile?.gender_id === 0 
          ? "ប្រុស" 
          : item.profile?.gender_id === 1 
            ? "ស្រី" 
            : "N/A",
      ...
      const BOM = "\uFEFF";
      const csvContent =
        BOM + [headers.join(","), ...data.map((row) => row.join(","))].join("\n");
