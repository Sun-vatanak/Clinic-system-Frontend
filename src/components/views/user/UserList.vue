      ...
      <table class="table">
        ...
        <tbody>
          ...
          <template v-if="!userStore.isLoading">
            <tr
              v-for="(user, i) in userStore.data_users"
              :key="user.id"
              class="align-middle"
            >
              <td>
              ...
              <td>
                <div class="d-flex align-items-center gap-2">
                  <div class="profile_user bg-light overflow-hidden">
                    <img :src="user.profile?.photo || '/default.png'" class="card-img" alt="" />
                  </div>
              ...
              <td class="fs-4 text-primary">
                <i 
                  class="bi-toggle-on status-user" 
                  v-if="user.is_active"
                  @click="onclickStatus(user)"
                ></i>
                <i 
                  class="bi-toggle-off status-user"
                  v-else
                  @click="onclickStatus(user)"
                ></i>
              </td>
              <td>
                <div class="d-flex gap-3">
                  <a
                    role="button"
                    class="btn-action btn-views"
                    @click="onclickViewsUser(user)"
                    title="មើលលម្អិត"
                  >
                    <i class="bi bi-eye"></i>
                  </a>
