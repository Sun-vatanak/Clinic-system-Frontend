        ...
        <div class="modal-body">
          <div class="d-flex align-items-center gap-3 mb-4">
            <div class="form-photo overflow-hidden mb-3">
              <div
                class="from-photo bg-light rounded-circle overflow-hidden"
                :class="{
              ...
              >
                <img
                  :src="
                    categoryStore.crop.avatar &&
                    typeof categoryStore.crop.avatar === 'string' &&
                    categoryStore.crop.avatar.startsWith('blob:')
                      ? categoryStore.crop.avatar
                      : categoryStore.crop.avatar &&
                        typeof categoryStore.crop.avatar === 'string'
                      ? `http://clinic-management-system.test/${categoryStore.crop.avatar}?${categoryStore.crop.avatar}`
                      : '/default.png'
                  "
                  class="card-img"
                  alt="Category Photo"
              ...
              <div
                class="invalid-feedback d-block ps-2 position-absolute bottom-1"
                v-if="categoryStore.v_validate.photo.$error"
              >
          ...
          <div>
            <form @submit.prevent="onclickAddMarket()">
              <div class="mb-4 w-100">
                <div class="input-field">
                  <input
                    ...
                    id="name"
                    v-model="categoryStore.frm.name"
                    placeholder=""
                ...
                <div
                  ...
                  v-if="categoryStore.v_validate.name.$error"
                >
              ...
              <div class="mb-4 w-100">
                <div class="input-field">
                  <textarea
                    ...
                    class="form-control rounded-5"
                    id="name"
                    v-model="categoryStore.frm.description"
                    placeholder=""
                  />
                  <label for="name" class="form-label">ប្រភេទផលិតផល</label>
                </div>
              ...
              <div class="w-100 text-end">
                <button
                  type="button"
                  class="btn btn-light rounded-5 btn-150 text-cancel me-2"
                ...
                >
                  <span v-if="!isLoading" role="status">{{
                    categoryStore.selectedId == 0 ? " បញ្ជូន" : "កែប្រែ"
                  }}</span>
                  <span
                    v-if="isLoading"
...
const onSelectedImage = (e) => {
 ...
 const allowedTypes = ["image/jpeg", "image/png", "image/webp", "image/gif"];
  ...
  if (file.size > maxSize) {
    categoryStore.error_message = "ទំហំរូបភាពមិនត្រូវលើស 4MB";
    categoryStore.mdl_add.hide();
    categoryStore.modalErrorTypeImage.show();
    return;
  }
...
const onclickAddMarket = async () => {
  ...
  frmData.append("description", categoryStore.frm.description);
  ...
  if (categoryStore.selectedId == 0) {
    try {
      ...
      if (err.response?.data?.errors?.name?.[0]) {
        ...
        if (
          err.response.data.errors.name[0].includes("The name has already been taken.")
        ) {
    ...
    try {
      const res = await axios.post(
        `/api/categories/${categoryStore.selectedId}`,
        frmData
      );
      if (res.data.result) {
        categoryStore.mdl_add.hide();
