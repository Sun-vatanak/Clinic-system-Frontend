...
onMounted(() => {
  vendorStore.mdl_crop = Modal.getOrCreateInstance(
    document.getElementById("mdl-crop")
  );
});

const chooseFileImage = () => {
  vendorStore.mdl_crop.hide();
...
const onCropted = () => {
  temp.toBlob((bin) => {
    vendorStore.crop.blob = bin;
    vendorStore.crop.avatar = URL.createObjectURL(bin);
    vendorStore.mdl_crop.hide();
