      ...
      <div class="modal-dialog modal-sm modal-dialog-centered">
          <div class="modal-content rounded-4 p-2">
              <div class="modal-body d-flex flex-column align-items-center gap-3">
                  ...
                  <div>
                      <p class="mb-0 text-center fs-4  fw-semibold mb-1">លុបអ្នកលក់</p>
                      <p class="mb-0 text-center  fs-6 text-form">អ្នកនឹងលុប <span class="text-black">{{ vendorStore.frm.business_name }}</span> ចេញពីប្រព័ន្ធ។ តើអ្នកប្រាកដទេ?</p>
                  </div>
...
onMounted(() => {
  vendorStore.mdl_delete = Modal.getOrCreateInstance(document.getElementById('mdl-del'));
});

const onclickDelete = async () => {
  isLoading.value = true;
  try {
      await axios.delete(`/api/vendors/${vendorStore.selectedId}`);
      vendorStore.onloadVendor();
      vendorStore.mdl_delete.hide();
  } catch (error) {
      console.error(error);
  } finally {
